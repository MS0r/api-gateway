...
async def test_course(session: AsyncSession) -> course.Course:
    course_created = course.Course(
        title="Test Course 1", description="This is a test course."
    )
...
async def test_quiz(session: AsyncSession, test_subunit: subunit.Subunit) -> quiz.Quiz:
    ...
    quiz_question = quiz.QuizQuestion(
        question_text="What is the capital of France?", quiz_id=quiz_created.id
    )
    session.add(quiz_question)
    await session.commit()
    await session.refresh(quiz_question)
    option = quiz.Option(
        text="Paris", is_correct=True, quiz_question_id=quiz_question.id
    )
    session.add(option)
    await session.commit()
    await session.refresh(quiz_created)
...
@pytest_asyncio.fixture
async def test_quiz_pass(
    session: AsyncSession, test_user: user.User, test_quiz: quiz.Quiz
) -> quiz_pass.QuizPass:
    quiz_pass_created = quiz_pass.QuizPass(user_id=test_user.id, quiz_id=test_quiz.id)
...
async def test_exercise(
    session: AsyncSession, test_unit: unit.Unit
) -> exercise.Exercise:
...
async def test_submission(
    session: AsyncSession, test_exercise: exercise.Exercise, test_user: user.User
...
async def test_question(
    session: AsyncSession, test_user: user.User
...
async def test_answer(
    session: AsyncSession, test_user: user.User, test_question: publication.Question
) -> publication.Answer:
    answer_created = publication.Answer(
        body="Response", user_id=test_user.id, question_id=test_question.id
    )
...
# Vote fixtures
@pytest_asyncio.fixture
async def question_upvote(
    session: AsyncSession, test_user: user.User, test_question: publication.Question
) -> vote.Vote:
    vote_created = vote.Vote(
        user_id=test_user.id, question_id=test_question.id, vote=vote.VoteType.UPVOTE
    )
...
@pytest_asyncio.fixture
async def question_downvote(
    session: AsyncSession, test_user: user.User, test_question: publication.Question
) -> vote.Vote:
    vote_created = vote.Vote(
        user_id=test_user.id, question_id=test_question.id, vote=vote.VoteType.DOWNVOTE
    )
    session.add(vote_created)
    await session.commit()
    await session.refresh(vote_created)
...
@pytest_asyncio.fixture
async def answer_upvote(
    session: AsyncSession, test_user: user.User, test_answer: publication.Answer
) -> vote.Vote:
    vote_created = vote.Vote(
        user_id=test_user.id, answer_id=test_answer.id, vote=vote.VoteType.UPVOTE
    )
    session.add(vote_created)
    await session.commit()
    await session.refresh(vote_created)
...
@pytest_asyncio.fixture
async def answer_downvote(
    session: AsyncSession, test_user: user.User, test_answer: publication.Answer
) -> vote.Vote:
    vote_created = vote.Vote(
        user_id=test_user.id, answer_id=test_answer.id, vote=vote.VoteType.DOWNVOTE
    )
    session.add(vote_created)
    await session.commit()
    await session.refresh(vote_created)
...
async def token(test_user: user.User) -> str:
    ...
    return jwt.create_access_token_for_user(
        test_user, settings.secret_key.get_secret_value()
    )


@pytest.fixture
def authorized_client(
    client: AsyncClient, authorization_prefix: str, token: str
) -> AsyncClient:
    client.headers = {
