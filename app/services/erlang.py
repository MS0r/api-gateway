...
def send_data_sandbox(payload: dict) -> ErlangTestResponse | ErlangCompileResponse:
    try:
        ...
        if 'cases' in payload:
            return ErlangTestResponse.model_validate(response_data)
...
async def submit_code_erlang(db: AsyncSession, submission: SubmissionCreate) -> ErlangTestResponse:
    existing = await submission_crud.get_submissions_by_user_exercise(
        db, submission.exercise_id, submission.user_id
    )
    results = await test_code_erlang(db, submission.code_snippet, submission.exercise_id)
    if not existing and results.test_results.failures == 0 and results.status == "ok":
        created = await submission_crud.create_submission(db, submission)
        if not created:
